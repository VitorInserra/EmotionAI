...
class AccuracyBar(tk.Frame):
    def __init__(self, master: tk.Misc, width: int = 320, **kwargs) -> None:
        ...
        self.title_label = tk.Label(self, text="Model Accuracy", font=("Helvetica", 15, "bold"), bg=BG_COLOR, fg=FG_COLOR)
        self.title_label.pack(anchor="e", pady=(0, 6))
        self.canvas = tk.Canvas(self, width=self.width, height=self.height, bg=BG_COLOR, highlightthickness=0, bd=0)
        self.canvas.pack(anchor="e", fill="x")
        self.value_label = tk.Label(self, text="0.0%", font=("Helvetica", 13), bg=BG_COLOR, fg=FG_COLOR)
...
class QuadrantMap(tk.Frame):
    def __init__(self, master: tk.Misc, title: str, **kwargs) -> None:
        ...
        self.title_label = tk.Label(self, text=title, font=("Helvetica", 16, "bold"), bg=PANEL_COLOR, fg=FG_COLOR)
        self.title_label.pack(anchor="center", pady=(0, 10))
        self.canvas = tk.Canvas(self, width=self.map_size, height=self.map_size, bg=PANEL_COLOR, highlightthickness=0, bd=0)
        self.canvas.pack()
        self._draw_base_map()
...
class PictureQuadrantGame:
    ...
    def _build_layout(self) -> None:
        ...
        self.info_label = tk.Label(
            self.info_panel,
            text="Look at the image, then press 1-4 to classify your emotion. Press Enter for the next image. Press Q to append current_session.csv into full_dataset.csv and quit.",
            justify="left",
            anchor="w",
            font=("Helvetica", 13),
            bg=PANEL_COLOR,
        ...
        self.status_label = tk.Label(self.info_panel, text="EEG stream ready.", justify="left", anchor="w", font=("Helvetica", 11), bg=PANEL_COLOR, fg=MUTED_TEXT_COLOR, wraplength=340)
    ...
    def _refresh_layout(self) -> None:
        ...
        self.root.configure(padx=max(8, min(18, root_w // 90)), pady=max(8, min(18, root_w // 90)))
    ...
    def _refresh_status_text(self) -> None:
        ...
        self.status_label.configure(
            text=(
                f"EEG: {self.eeg_stream.status_message}\n"
                f"Sensor contact quality: {contact_text}\n"
                f"Model: {self.model.status_text()}\n"
                f"Fine-tune: {self.finetune_status_text}\n"
    ...
    def _build_rows_for_trial(
        ...
        normalized_samples = samples or [{"timestamp": trial_ended_epoch}]
        for sample in normalized_samples:
            ...
            row: dict[str, object] = {
                ...
                "sample_timestamp": datetime.fromtimestamp(sample_ts, tz=timezone.utc).isoformat(),
    ...
    def _predict_model_choice(self, samples: list[dict[str, float]]) -> Optional[Quadrant]:
        code = self.model.predict_code(samples)
    ...
    def _maybe_start_finetune(self) -> None:
        ...
        if not script_path.exists():
            ...
            return
        cmd = [sys.executable, str(script_path), "--round-count", str(self.trial_index), "--current-session", str(self.recorder.current_session_path)]
        self.finetune_process = subprocess.Popen(cmd, cwd=str(self.base_dir), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, text=True)
    ...
    def _handle_quadrant_key(self, event: tk.Event) -> None:
        ...
        samples = self.eeg_stream.samples_between(self.trial_started_epoch, trial_ended_epoch, fallback_latest=True)
        ...
        model_choice = self._predict_model_choice(samples) if samples else None
        ...
        print(f"Saved {len(rows)} row(s) to {self.recorder.current_session_path}", flush=True)
